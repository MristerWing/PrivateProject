...
console.log(arr, sum); //누적값이 undefind라면 인덱스의 첫번째 값이 누적값의 시작값이 된다.

const sum2 = arr.reduce((a, x) => a += x);
console.log(sum2); //객체 또한 누적값으로 이용할 수 있다.
...
console.log(alphabetical); // reduce 통계
...
const words2 = ["Beachball", "Rodeo", "Angel", "Aardvark", "Xylophone", "November", "Chocolate", "Papaya", "Uniform", "Joker", "Clover", "Bail"];
const longWords = words2.reduce((a, w) => w.length > 6 ? a + " " + w : a, "").trim();
const filterAndJoin = words2.filter(w => w.length > 6).join(" ").trim();
