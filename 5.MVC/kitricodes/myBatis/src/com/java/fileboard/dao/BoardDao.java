...
public class BoardDao {
	...
	public int insert(BoardDto boardDto) {
		...
		try {
			if (boardDto.getFileSize() == 0) {
				...
				value = session.insert("fileboardInsert", boardDto);
				session.commit();
			} else {
				session = sqlSessionFactory.openSession();
				value = session.insert("fileboardInsertFile", boardDto);
				session.commit();
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void writeNumber(BoardDto boardDto) {
		...
		try {
			...
			if (boardNumber == 0) {
				...
				session = sqlSessionFactory.openSession();
				boardGroupNumberMax = session.selectOne("fileboardGroupNumberMax");
			...
			// 답글 - 글순서, 글레벨
				...
				session = sqlSessionFactory.openSession();
				session.update("fileSequenceNumberUpdate", map);
				session.commit();
	...
	public List<BoardDto> getBoardList(int startRow, int endRow) {
		...
		try {
			session = sqlSessionFactory.openSession();
			boardList = session.selectList("fileboardList", map);
	...
	public BoardDto read(int boardNumber) {
		...
		try {
			...
			// update
			session = sqlSessionFactory.openSession();
			session.update("fileboardReadCountUpdate", boardNumber);
			session.commit();
	...
	public BoardDto getBoard(int boardNumber) {
		...
		try {
			session = sqlSessionFactory.openSession();
			boardDto = session.selectOne("fileboardGetBoard", boardNumber);
	...
	public int delete(int boardNumber) {
		...
		try {
			session = sqlSessionFactory.openSession();
			check = session.delete("fileboardDelete", boardNumber);
			session.commit();
