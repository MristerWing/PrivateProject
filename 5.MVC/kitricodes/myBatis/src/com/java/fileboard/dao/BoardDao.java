	...
	public int insert(BoardDto boardDto) {
		...
		try {
			...
			if (boardDto.getFileSize() == 0) {
				// 생략되야되는 필드명이 있으면 명시해야함. 까먹지 말것
				String sql = "insert into board(board_number, writer, subject, email, content, password, write_date, read_count, group_number, sequence_number, sequence_level) values"
	...
	public int update(BoardDto boardDto) {
		...
		try {
			if (boardDto.getFileName() != null) {
				String sql = "Update board set writer = ?, subject = ?, content = ?, write_date = ?, file_name = ?, path = ?, file_size = ? where board_number = ? and password = ?";
				conn = ConnectionProvider.getConnection();
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, boardDto.getWriter());
				pstmt.setString(2, boardDto.getSubject());
				...
				pstmt.setString(5, boardDto.getFileName());
				pstmt.setString(6, boardDto.getPath());
				pstmt.setLong(7, boardDto.getFileSize());
				pstmt.setInt(8, boardDto.getBoardNumber());
				pstmt.setString(9, boardDto.getPassword());
