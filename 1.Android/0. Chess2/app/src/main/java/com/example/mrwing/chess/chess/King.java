...
public class King extends Square{
    ...
    public boolean moveCheck(int[] from, int[] to, String plyColor, boolean testKing) {
        ...
        if(toX == fromX + 2) {
            if(this.getCastling() && ChessView.board[fromY][7].getCastling()) {
                for(int i = fromX+1; i < 7; i++) {
                    if(ChessView.board[fromY][i].getType() != "none") {
                        return false;
                    }
                }
                ChessView.board[fromY][7].setX(5);
                ChessView.board[fromY][7].setY(fromY);
                ChessView.board[fromY][5] = ChessView.board[fromY][7];
                ChessView.board[fromY][7] = new BlankSpace();
            ...
            if(this.getCastling() && ChessView.board[fromY][0].getCastling()) {
                for(int i = fromX-1; i > 0; i--) {
                    if(ChessView.board[fromY][i].getType() != "none") {
                        return false;
                    }
                }
                ChessView.board[fromY][0].setX(3);
                ChessView.board[fromY][0].setY(fromY);
                ChessView.board[fromY][3] = ChessView.board[fromY][0];
                ChessView.board[fromY][0] = new BlankSpace();
                this.setCastling(false);
                y = toY;
                x = toX;
                return true;
    ...
    public ArrayList<ArrayList<Integer>> movePathCheck() {
        ...
        for(int i = y - 1; i < y + 2; i++) {
            for(int j = x - 1; j < x + 2; j++) {
                if(i > 7 || i < 0 || j > 7 || j < 0) continue;
                if(i == y && j == x) continue;
                if(ChessView.board[i][j].getColor() != this.getColor()) {
                    ArrayList<Integer> tmp = new ArrayList<Integer>(Arrays.asList(i, j));
                    movePath.add(tmp);
                }
            }
        ...
        //케슬링 퀸사이드
        if(this.getCastling()) {
            if(ChessView.board[y][x - 4].getCastling()) {
                for(int i = x - 1; i > 0; i--) {
                    if(ChessView.board[y][i].getType() != "none") {
                        count++;
                    }
                }
                if(count == 0) {
        ...
        count = 0;
        if(this.getCastling()) {
            if(ChessView.board[y][x + 3].getCastling()) {
                for(int i = x + 1; i < 7; i++) {
                    if(ChessView.board[y][i].getType() != "none") {
                        count++;
                    }
    ...
    public ArrayList<ArrayList<Integer>> aiPath(){
        ...
        for(int i = y - 1; i < y + 2; i++) {
            for(int j = x - 1; j < x + 2; j++) {
                ...
                if(i == y && j == x) continue;
                if(ChessView.board[i][j].getColor() != this.getColor()) {
                    //if(ChessView.board[i][j].getType().equals("king"))
                    boolean kingCantKillKing = false;
                    if(getColor().equals("black")) {
                        for (ArrayList<Integer> key : ChessView.wKAiPath) {
                            if(key.contains(new ArrayList<>(Arrays.asList(i, j)))) {
                                kingCantKillKing = true;
                                break;
                            }
                        }
                    }else {
                        for (ArrayList<Integer> key : ChessView.bKAiPath) {
                            if(key.contains(new ArrayList<>(Arrays.asList(i, j)))) {
                                kingCantKillKing = true;
                                break;
                            }
        ...
        if(this.getColor() == "white"){
            tmp = ChessView.allBAttackPath;
            //tmp.addAll(ChessView.board[ChessView.bKYX.get(0)][ChessView.bKYX.get(1)].aiPath());
        }else{
            tmp = ChessView.allWAttackPath;
            //tmp.addAll(ChessView.board[ChessView.wKYX.get(0)][ChessView.wKYX.get(1)].aiPath());
        }
